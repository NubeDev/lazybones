...
use lazybones_ext::{ExtEngine, Registry};
...
/// `document.render` resolves logo/image bytes through the same backend. The
/// `extensions` registry + `ext_engine` cell are shared too, so the loop-only
/// extension tools install/grant/enable/invoke against the **same** dispatch index +
/// engine the routes and the scheduler use (design §6.3); the engine cell stays a
/// lazy [`OnceLock`] so mounting never forces engine construction (its epoch ticker
/// thread spins up only on first `extension.invoke`). Sessions are held in rmcp's
...
pub fn streamable_http_service(
    ...
    StreamableHttpService::new(
        move || {
            Ok(McpServer::new(store.clone(), resolver.clone())
                .with_assets(assets.clone())
                .with_ext_runtime(extensions.clone(), ext_engine.clone()))
        },
